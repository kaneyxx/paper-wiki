...
def test_ensure_env_contains_shim_tag() -> None:
    ...
    body = script.read_text(encoding="utf-8")
    expected_tag = (
        "# paperwiki shim — v0.3.33 (shared venv + self-bootstrap + PYTHONPATH fallback)."
    )
...
def test_ensure_env_shim_integration(tmp_path: Path) -> None:
    ...
    assert (
        "paperwiki shim — v0.3.33 (shared venv + self-bootstrap + PYTHONPATH fallback)." in body
    ), "shim must contain the v0.3.33 expected tag line"
