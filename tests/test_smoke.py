...
def test_bash_helpers_present_with_v0_3_38_tag() -> None:
    ...
    Pins the helper file's existence, the v0.3.40 tag line (so a release
    ...
    assert "# paperwiki bash-helpers — v0.3.40" in body, (
        "helper must carry the v0.3.40 version tag line"
    )
...
def test_ensure_env_contains_shim_tag() -> None:
    ...
    assert expected_tag in body, (
        "hooks/ensure-env.sh must contain the v0.3.40 shim tag line so "
...
def test_ensure_env_shim_integration(tmp_path: Path) -> None:
    ...
    assert (
        "paperwiki shim — v0.3.40 (shared venv + self-bootstrap + PYTHONPATH fallback)." in body
    ), "shim must contain the v0.3.40 expected tag line"
