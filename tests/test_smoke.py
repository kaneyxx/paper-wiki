...
def test_ensure_env_contains_shim_tag() -> None:
    ...
    body = script.read_text(encoding="utf-8")
    expected_tag = (
        "# paperwiki shim — v0.3.35 (shared venv + self-bootstrap + PYTHONPATH fallback)."
    )
    assert expected_tag in body, (
        "hooks/ensure-env.sh must contain the v0.3.35 shim tag line so "
...
def test_ensure_env_shim_integration(tmp_path: Path) -> None:
    ...
    assert (
        "paperwiki shim — v0.3.35 (shared venv + self-bootstrap + PYTHONPATH fallback)." in body
    ), "shim must contain the v0.3.35 expected tag line"
