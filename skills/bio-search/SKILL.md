...
- The user types `/paperwiki:bio-search <query>`.
...
- arXiv / general CS work — that routes to `/paperwiki:digest` with
  the `daily-arxiv` recipe.
- Single-paper deep dives — that routes to `/paperwiki:analyze`.
...
3. **Call paperclip via MCP.** Use the `mcp__paperclip__search` tool
   ...
   actual abstracts.
4. **Offer follow-ups.**
   - "Analyze paper #N in depth" → invoke `/paperwiki:analyze
...
- `/paperwiki:wiki-lint` surfaces a `DANGLING_SOURCE` finding for any
  newly saved source until the user runs `/paperwiki:wiki-ingest`.
